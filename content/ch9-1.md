...
# 強迫 ill-behaved 程式 completion。
# Useful, for example, in init scripts.
# 在 init scripts 中是相當實用的。

# Thank you, Sylvain Fourmanoit, for this creative use of the "!" variable.
# 感謝 Sylvain Fourmanoit 告訴我們 ! 變數的創意用法。
```
...
possibly_hanging_job & {
        while ((count < TIMEOUT )); do
                ...
                # 運行中的 process 資訊可在 /proc 中被找到。 
                # "-d" tests whether it exists (whether directory exists).
                # "-d" 參數測試資料夾是否存在。
                # So, we're waiting for the job in question to show up.
                # 我們等候 job in question 的出現。
                ((count++))
                sleep 1
        done
        eval '[ -d "/proc/$!" ] && kill -15 $!'
        # If the hanging job is running, kill it.
        # 如果 hanging job 正運行著，就砍掉該 process。
}
...
echo $_              #  /bin/bash
                     ...
                     #  注意，依據不同觸發腳本的方式，運行結果會有所不同。

du >/dev/null        #  So no output from command.
					 #  command 執行結果無 output。
echo $_              #  du
...
>`指令, 函式或腳本本身的退出狀態 (詳見範例 24-7)`

$$
Process ID (PID) of the script itself.
>`腳本本身的 PID。`

[5] The $$ variable often finds use in scripts to construct "unique" temp file names (see Example 32-6, Example 16-31, and Example 15-27). 
>`[5] 變數 $$ 通常在腳本中會用來建構非重複的 temp file names。(詳見範例 32-6, 範例 16-31 以及範例 15-27)。`

This is usually simpler than invoking mktemp.
>`通常這會比 invoking mktemp 還簡單。`

Notes

[1]	
A stack register is a set of consecutive memory locations, such that the values stored (pushed) are retrieved (popped) in reverse order. 
>`stack register 是一組連續的 memory locations，取值的順序與進入 stack 的順序是相反的。`

The last value stored is the first retrieved. 
>`最後被儲存的值，將是第一個取出的。`

This is sometimes called a LIFO (last-in-first-out) or pushdown stack.
>`有時此方法被稱為 LIFO (後進掀出) 或 pushdown stack 法。`

[2]	
The PID of the currently running script is $$, of course.
>`當然，現前運行腳本的 PID 值可從變數 $$ 中取得。`

[3]	
Somewhat analogous to recursion, in this context nesting refers to a pattern embedded within a larger pattern. 
>`與遞迴類似，此處 context nesting 參考了 pattern embedded within a larger pattern。`

One of the definitions of nest, according to the 1913 edition of Webster's Dictionary, illustrates this beautifully: "A collection of boxes, cases, or the like, of graduated size, each put within the one next larger."
>`依據 1913 年版的 Webster 字典優美描述，nest 其中一種定義就是：許多盒子、箱子或類似物，依據大小，依序將次小放入其中。`

[4]	
The words "argument" and "parameter" are often used interchangeably. 
>`argument 與 parameter 經常交換著使用。`

In the context of this document, they have the same precise meaning: a variable passed to a script or function.
>`在此文件中，他們都有一個相同精準的定義，一個被傳遞至函式或腳本的變數。`

[5]	
Within a script, inside a subshell, $$ returns the PID of the script, not the subshell.
>`在腳本中，subshell 裡，$$ 變數會回傳 腳本的 PID 而非 subshell 的。`

ref: http://tldp.org/LDP/abs/html/internalvariables.html
>`參考文獻：http://tldp.org/LDP/abs/html/internalvariables.html`
