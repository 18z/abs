...
>`在[[]]中，無 filename expansion 或 word splitting 之功能，但有 parameter expansion 及 command substitution 之功能。`
...
then
  ...
  echo "$decimal is not equal to $hex"
        ...
        # [[ $hexadecimal (十六進制) ]] 一樣會進行判斷！
```

Following an if, neither the test command nor the test brackets ( [ ] or [[ ]] ) are strictly necessary.
>`if 後面，嚴格要求需放至測試指令 (test command) 或測試引號 (test brackets [ ... ] 或 [[ ... ]])。`

```bash
dir=/home/bozo

if cd "$dir" 2>/dev/null; then   # "2>/dev/null" hides error message.
                                 # 「2>/dev/null」會將錯誤訊息藏起來。
  echo "Now in $dir."            # 現在位置 $dir 資料夾。
else
  echo "Can't change to $dir."   # 無法切換資料夾。
fi
```

The "if COMMAND" construct returns the exit status of COMMAND.
>`if COMMAND 結構，會將 COMMAND 的退出狀態 (exit status) 回傳。`

Similarly, a condition within test brackets may stand alone without an if, when used in combination with a list construct.
>`與 list construct 結合使用時，可單獨用 [ ... ]來進行測試。`
...
>`若運算結果為零，則回傳退出狀態 (exit status) 1 或 false 。`
...
[1]	A token is a symbol or short string with a special meaning attached to it (a meta-meaning).
>`特殊符號 (token)，為具有特殊意義之短字串或符號 (symbol)者。`

In Bash, certain tokens, such as [ and . (dot-command), may expand to keywords and commands.
>`在 Bash 中，特定特殊符號如 [ 及 .，皆為指令或為特定關鍵字代表符號。`
